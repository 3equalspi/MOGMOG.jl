...
function (encoder::MOGencoder)(
    ...
    index_embedding = encoder.index_embed(as_dense_on_device((1:L-1)' ./ 10f0, positions))
    # D x L x B --> D x 5 x L x B
    with_next_atom = (atom_embedding + encoder.next_atom_embed(atom_types[2:L, :])) .+ index_embedding
    with_next_x = index_embedding .+ encoder.next_x_embed(positions[1:1, 2:L, :]) 
    with_next_y = index_embedding .+ encoder.next_y_embed(positions[2:2, 2:L, :])
    with_next_z = index_embedding .+ encoder.next_z_embed(positions[3:3, 2:L, :])
